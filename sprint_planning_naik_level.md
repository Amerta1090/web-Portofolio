...
| L4.5 | Verifikasi & tutup Iterasi 4 | ✅ | Ringkasan Iterasi 4 — **Warna, Kontras & Material Depth**: (1) Token `--color-brand-text` (#5d6b54 light / #7a8c6f dark) + remap `--color-brand-rgb` → all ~100+ `text-brand` usages pass WCAG AA 4.51:1+; (2) Noise overlay SVG feTurbulence 3% opacity CSS-only, tier-1 hidden; (3) `.card` dark inset highlight `rgba(255,255,255,0.06)` border + `inset 0 1px 0 rgba(255,255,255,0.08)` + `.card-material` utility; (4) 3-level elevation `--shadow-1/2/3` tokens applied to Toaster/modal/tooltip. Build 45 ✓, unit 406/406 ✓, E2E 22/22 ✓ (craft 4 + typography 4 + fonts 5 + micro-interactions 9) |
| L5.1 | Template case study + content collection | ✅ | Content collection `caseStudies` di `src/content.config.ts` (Zod schema: title/summary/role/stack/period/category/metrics/featured/order); schema testable di `src/content/schema.ts`; 2 placeholder MDX di `src/content/case-studies/`; route `/work/[slug]` editorial layout (65ch, display font Fraunces, metrics row, stack tags, prev/next nav); listing `/work`; 14 unit test + 6 E2E. Build 48 ✓, unit 420/420 ✓, E2E 6/6 ✓ + 22 existing ✓ |
| L5.2 | Tulis 2–3 case study mendalam | ✅ | 2 case study ditulis: AI Quranic Tafsir Assistant (RAG + Hybrid Retrieval + LLaMA 3.2, metrics: context relevance 0.972, latency 5.93s) + Red Devil Dynamics (Survival Analysis + Goal Timing, metrics: 10-min prediction window, live deployment). Placeholder lama dihapus. Build 48 ✓, unit 420/420 ✓, E2E 6/6 ✓ |
| L5.3 | Kurasi Featured Lab di /gallery | ✅ | `featured?: boolean` field ditambah ke `Experiment` interface; 5 eksperimen ditandai: Liquid Distortion, Audio Visualizer, Fractal Explorer, Interactive Canvas, Galaxy Formation; section "Featured" muncul di atas grid saat filter "All" aktif, menggunakan ExperimentCard tanpa keyboard nav integration (regular grid tetap handle nav). Build 48 ✓, unit 420/420 ✓, E2E 14/14 ✓ (gallery WebGL flake pre-existing) |
| L5.4 | OG image dinamis per halaman | ✅ | `astro-og-canvas` package; route `src/pages/og/[...route].ts` generate 1200×630 PNG per halaman (dark bg #0e0f0c, white title, sage accent #5d6b54); `ogSlug` prop di BaseLayout + 14 halaman ter-update; ~34 unique OG PNGs di `dist/og/`. Build 48 ✓ (+1m11s, overhead <30s), unit 420/420 ✓ |
| L5.5 | Verifikasi & tutup Iterasi 5 | ✅ | Ringkasan Iterasi 5 — **Konten, Kurasi & Narasi**: (1) Content collection `caseStudies` + Zod schema + `/work/[slug]` editorial route (65ch, display font, metrics, tags); (2) 2 case studies ditulis (AI Quranic Tafsir RAG + Red Devil Dynamics Survival Analysis); (3) Featured Lab section di `/gallery` (5 curated: LiquidDistortion, AudioVisualizer, FractalExplorer, InteractiveCanvas, GalaxyFormation); (4) Dynamic OG images via `astro-og-canvas` (~34 PNGs, 1200×630, dark+sage theme). Build 48 ✓, unit 420/420 ✓, E2E 28/28 ✓ |
