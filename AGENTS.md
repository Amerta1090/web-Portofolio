...
- Sprint Motion (motion.dev upgrade + MotionScore): 🔄 Phase A–C ✅ COMPLETE, Phase D ⏳ [BUTUH USER] — migrasi framer-motion→motion 13.4.1, RAF off-screen guard (`useRafGuard`), MotionScore pasca-C: `/` B 53 · `/gallery` S 86 · `/observatory` A 77 (nol temuan D/F). Detail di bawah.
...
### Sprint Motion (motion.dev upgrade + MotionScore) — Phase A–C ✅ COMPLETE, Phase D ⏳ [BUTUH USER]
...
- **Phase D ⏳ [BUTUH USER]**: (1) MotionScore Guard workflow gratis utk comment (`deployment_status`) — gate threshold berbayar jangan dipasang tanpa persetujuan; (2) `animateView` morph kartu→modal GalleryGrid; (3) Motion UI / Motion+ (berbayar) — hanya bila lisensi ada.
